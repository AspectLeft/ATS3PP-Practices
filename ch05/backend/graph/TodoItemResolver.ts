...
export class TodoItemResolver implements ResolverInterface<TodoItem> {
  ...
  async TodoItem(@Arg("title") title: string): Promise<TodoItem | undefined> {
    return Prefill.Instance.Items.find(todoItem => todoItem.Title === title);
  }

  @Query(() => [TodoItem], { description: "Get all the TodoItems" })
  async TodoItems(): Promise<TodoItem[]> {
    return Prefill.Instance.Items;
  }
  ...
  async Add(@Arg("TodoItem") todoItemInput: TodoItemInput): Promise<TodoItem> {
    ...
    await this.dataAccess.Add(TodoItemResolver.CreateTodoSchema(todoItem));
    return todoItem;
  }

  private static CreateTodoSchema<T extends TodoItem | TodoItemInput>(todoItem: T): ITodoSchema {
    return <ITodoSchema>{
      Id: todoItem.Id,
  ...
  async Update(@Arg("TodoItem") todoItemInput: TodoItemInput): Promise<boolean> {
    const item: TodoItem | undefined = await Prefill.Instance.Items.find(x => x.Id === todoItemInput.Id);
    if (!item) return false;
    item.Title = todoItemInput.Title;
    item.Description = todoItemInput.Description;
    item.DueDate = todoItemInput.DueDate;
    await this.dataAccess.Update(item.Id, TodoItemResolver.CreateTodoSchema(item));
    return true;
  }

  @Mutation(() => Boolean!)
  async Complete(@Arg("Id") id: string) : Promise<boolean> {
    const item: TodoItem | undefined = await Prefill.Instance.Items.find(x => x.Id === id);
    if (!item) return false;
    item.Completed = true;
    await this.dataAccess.Update(item.Id, TodoItemResolver.CreateTodoSchema(item));
    return true;
  ...
  DaysCreated(
    ...
    const value = TodoItemResolver.GetDateDifference([new Date(), TodoItem.CreationDate]);
  ...
  private static GetDateDifference(args: [Date, Date]): number {
